...
public class Item
{
            ...
            foreach (var item in registeredItems.Where(i => i.configurability != Configurability.Disabled))
            {
                ...
                if ((item.configurability & Configurability.Recipe) != 0)
                {
                    ...
                    foreach (var configKey in item.Recipes.Keys.DefaultIfEmpty(""))
                    {
                        ...
                        if (item.Recipes.ContainsKey(configKey) && item.Recipes[configKey].Crafting.Stations.Count > 0)
                        {
                            ...
                            {
                                Order = --order, Browsable = cfg.table.Value != CraftingTable.Disabled,
                                Category = localizedName
                            };
                            ...
                            ConfigEntry<string> itemConfig(string name, string value, string desc)
                            {
                                ConfigurationManagerAttributes attributes = new()
                                {
                                    ...
                                    Browsable = cfg.table.Value != CraftingTable.Disabled, Category = localizedName
                                };
                    ...
                    for (var i = 0; i < item.Conversions.Count; ++i)
                    {
                        ...
                        void UpdatePiece()
                        {
                            ...
                            var newPieceName = conversion.config.piece.Value is not ConversionPiece.Disabled
                                ...
                                : null;
                            if (item.conversions[index].m_from is null && (conversion.config.activePiece is not null ||
                                                                           conversion.config.activePiece !=
                                                                           newPieceName))
                            {
                        ...
                        conversion.config.input = config(englishName, $"{prefix} Conversion Input Item",
                            conversion.Input, $"Duration of conversion to create {englishName}");
                        ...
                        conversion.config.piece = config(englishName, $"{prefix} Conversion Piece", conversion.Piece,
                            $"Duration of conversion to create {englishName}");
                        ...
                        conversion.config.customPiece = config(englishName, $"{prefix} Conversion Custom Piece",
                            conversion.customPiece ?? "", $"Duration of conversion to create {englishName}");
                ...
                if ((item.configurability & Configurability.Stats) != 0)
                {
                        ...
                        Action<ItemDrop.ItemData.SharedData, T> setValue)
                    {
                        ...
                        var itemName = shared.m_name;
                        cfg.SettingChanged += (_, _) =>
                        {
                            setValue(shared, cfg.Value);

                            var inventories = Player.m_players.Select(p => p.GetInventory())
                            ...
                            foreach (var itemdata in ObjectDB.instance.m_items.Select(p => p.GetComponent<ItemDrop>())
                                         .Where(c => c && c.GetComponent<ZNetView>()).Concat(ItemDrop.m_instances)
                    ...
                    {
                        statcfg("Food", $"Food value of {englishName}.", shared => shared.m_food,
                            (shared, value) => shared.m_food = value);
                        statcfg("Food Stamina", $"Food stamina value of {englishName}.", shared => shared.m_foodStamina,
                            (shared, value) => shared.m_foodStamina = value);
                        statcfg("Food Eitr", $"Food eitr value of {englishName}.", shared => shared.m_foodEitr,
                            (shared, value) => shared.m_foodEitr = value);
                        statcfg("Food Burn Time", $"Food burn time of {englishName}.", shared => shared.m_foodBurnTime,
                            (shared, value) => shared.m_foodBurnTime = value);
                        statcfg("Food Regen", $"Food regen value of {englishName}.", shared => shared.m_foodRegen,
                        ...
                        or ItemDrop.ItemData.ItemType.TwoHandedWeaponLeft)
                    {
                        ...
                        statcfg("Eitr Regen Modifier", $"Eitr Regen Modifier of {englishName}.",
                            shared => shared.m_eitrRegenModifier,
                            (shared, value) => shared.m_eitrRegenModifier = value);
                    }
                    ...
                    {
                        statcfg("Tool Tier",
                            $"Tool Tier of {englishName}. Dictates if the object can cut down hard trees.",
                            shared => shared.m_toolTier, (shared, value) => shared.m_toolTier = value);
    ...
    internal static void Patch_ObjectDBInit(ObjectDB __instance)
    {
        ...
        foreach (var item in registeredItems)
        {
            ...
            itemCraftConfigs.TryGetValue(item, out var cfgs);
            foreach (var kv in item.Recipes)
            {
    ...
    public static void SnapshotItem(ItemDrop item, float lightIntensity = 1.3f, Quaternion? cameraRotation = null)
    {
        ...
        var visual = Object.Instantiate(item.transform.Find("attach").gameObject);
    ...
    {
        return gui.m_selectedRecipe.Value?.m_quality > 0;
    }
    ...
    private class SerializedRequirements
    {
        ...
        public static ItemDrop? fetchByName(ObjectDB objectDB, string name)
        {
            ...
            if (item == null) Debug.LogWarning($"The required item '{name}' does not exist.");
            return item;
        }
